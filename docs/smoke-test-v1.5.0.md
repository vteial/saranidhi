...
|---|----------|-------|----------|--------|
| H1 | Upgrade from existing profile | Open staging with a DB created on v4 (existing profile + journal + oracle history) | App loads; no crash/white screen; all existing data intact; `somatic_intervention_logs` table created | ✅ Pass |
| H2 | Fresh install | Clear all site data → open app | Onboarding runs; DB created fresh at v5; no migration errors | ✅ Pass |
| H3 | Somatic log persists | Complete one intervention (Section J) → reopen app | Logged session survives reload (table writable) | ✅ Pass |
| H4 | Existing features unaffected post-migration | After H1, use journal/oracle/streak | All read/write normally | ✅ Pass |
...
| A5 | Notice localized | A1/A2 in Tamil | Notice text in Tamil | ⏸️ Deferred |
...
| B3 | Re-verify after migration | Fresh onboarding on v1.5.0 build | Works as in v1.4.2 | ✅ Pass |
...
| F2 | Moved >5 km silent update | Override coords → reload | Location updates + one-time notice; bird unchanged | ⏸️ Deferred |
...
| # | Scenario | Steps | Expected | Result |
|---|----------|-------|----------|--------|
| K1 | 4:4:4:4 rhythm | Observe pacer in timer room | Circle grows (inhale) → holds → shrinks (exhale) → holds; ~4s each; phase label + second count | ✅ Pass |
| K2 | Phase labels localized | Tamil mode | Inhale/Hold/Exhale labels in Tamil | ✅ Pass |

### L. Cross-Lateral Correctness (Task 35.5) — (2 scenarios)

| # | Scenario | Steps | Expected | Result |
|---|----------|-------|----------|--------|
| L1 | Target Lunar (left) | Trigger intervention where expected = Lunar | Instruction: act on **right** side (contralateral) | ✅ Pass |
| L2 | Target Solar (right) | Trigger intervention where expected = Solar | Instruction: act on **left** side | ✅ Pass |
...
| # | Scenario | Steps | Expected | Result |
|---|----------|-------|----------|--------|
| N1 | App loads without errors | Open app | Dashboard loads; startup widgets stack cleanly | ✅ Pass |
| N2 | Journal log works | Journal → select nostril → log | Entry saved with alignment | ✅ Pass |
| N3 | Full Day Schedule | Today tab → schedule | 5 yamas with bird states | ✅ Pass |
| N4 | Tamil mode | Switch to Tamil | All labels render | ✅ Pass |
| N5 | About shows v1.5.0 | Settings → About card | Version = 1.5.0 | ✅ Pass |
...
|---------|-----------|------|----------|----------|------|
| H. Migration (CRITICAL) | 4 | 4 | 0 | 0 | 0 |
| A. Auto-Recalc Bird | 5 | 0 | 0 | 5 | 0 |
| B. Onboarding 3 tabs | 3 | 3 | 0 | 0 | 0 |
| C. Summary + Validation | 2 | 2 | 0 | 0 | 0 |
| D. Nostril test | 1 | 1 | 0 | 0 | 0 |
| E. Oracle delete | 1 | 1 | 0 | 0 | 0 |
| F. Geolocation | 2 | 0 | 1 | 1 | 0 |
| J. Intervention flow (CRITICAL) | 6 | 6 | 0 | 0 | 0 |
| K. Sama Vritti pacer | 2 | 2 | 0 | 0 | 0 |
| L. Cross-lateral | 2 | 2 | 0 | 0 | 0 |
| M. Somatic Tamil + cancel | 2 | 2 | 0 | 0 | 0 |
...
**Release decision:**
- [x] All critical + core pass → ready to proceed with release!
