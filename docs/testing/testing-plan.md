...
> **Reviewed:** v1.13.0-web · **Next review:** every release (test-count + scenarios).
...
| Sprint 28 | 0 | 410 | UI polish + UX consistency; no new test files |
| Sprint 29 | 0 | 410 | Terminology, PWA, tech debt; widget test un-skipped (stream settling fix) |
| Sprint 30 | 10 | 420 | ActionWindowsEngine unit tests (consolidation, Rahu overlap, bird state mapping) |
| Sprint 31 | 20 | 440 | OracleCompositeEngine + TaraCategory + HoraSwaraAffinity + NameBirdParser unit tests |
...
| Sprint 45 | +12 | 661 | Practice Sync polish: `backup_filename_test` (prefix / null / empty / <8-char no-RangeError / single-char / default-now, 6), `profile_card_refresh_test` (provider re-read after invalidation + card refresh without page reload, 2), `intro_screen_test` (EN+TA import-link render, Get-Started happy-path unaffected, import-before-onboarding adopts source `ownerId` + flips `onboardingCompleteProvider`, 4) |
| Sprint 46 | +0 (in-repo) | 661 | Web E2E harness lives in `vteial/saranidhi-e2e` (Playwright Step 0 + S1–S6, 3× green, 1m24s) — counted there, not in the Flutter suite |
| Sprint 47 | +16 | 677 | Practice Sync Phase 1: `pocketbase_sync_transport_test` (auth / pull-filter-by-ownerId / upsert-by-uuid / web-safe, 4), `practice_sync_engine_test` (pull→merge→push round-trip w/ mocked transport, idempotent no-op, owner-guard-on-pull 0-mutations, not-configured/not-authed/disabled no-ops, 6), `practice_sync_ui_test` (opt-in toggle, checkboxes default-on, Sync-now, offline error status, 4), `database_exporter_test` (+2: mergePracticeRows disjoint+idempotent, session round-trip) |
