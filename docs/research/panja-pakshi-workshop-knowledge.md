...
>
> **Progress:** 2/6 confirmed. ✅ **Both Tier-1 engine blockers RESOLVED** (owner-adjudicated against unanimous lineage sources; Pulippani/vedastro overruled):
> - **CONF-PP-001** → nakshatra partition is **5-6-5-5-6** (not the shipped 5-5-5-5-7); Pooram→Owl, Visakam→Crow, Uthiradam→Rooster.
> - **CONF-PP-002** → birth-star bird is a **single permanent Valarpirai table, no Krishna swap**; the waxing/waning swap applies only to the name-initial fallback.
>
These two require a **`PakshiCalculator` engine refactor + existing-user birth-bird re-migration** (a Phase-2b task — see backlog; carries the v1.4.1 on-load-recalc lesson).
>
> ✅ **ALL 6 CONF-PP RESOLVED (6/6).** The remaining four are also owner-confirmed:
> - **CONF-PP-003** → unified friend/enemy matrix (Vulture allies Peacock+Owl; enemies Crow+Rooster).
> - **CONF-PP-004** → ruling planets Vulture=Jupiter, Owl=Venus, Crow=Mars, Rooster=Mercury, Peacock=Saturn.
> - **CONF-PP-005** → phase-dependent (dynamic) bird directions, not static.
> - **CONF-PP-006** → support BOTH sub-yama models; default equal 28.8-min (workshop), classical weighted as a user option.
>
> All engine changes are captured as Phase-2b `pakshi_attributes.dart` / `PakshiCalculator` tasks in the backlog. **Panja Pakshi corpus is now fully captured + CONF-resolved.**
...
|:--:|:--:|------------------|----------------------|--------|------------------|:----------:|:------:|
| CONF-PP-001 | 🔴 T1 | Nakshatra-to-Bird Star Boundaries | The shipped engine (`PakshiCalculator` / Pulippani) partitions the 27 nakshatras as 5-5-5-5-7 (Vulture=5, Owl=5, Crow=5, Cock=5, Peacock=7), assigning Purva Phalguni to Crow, Vishakha to Cock, and Uttara Ashadha to Peacock. The 2025 workshop master partitions stars as 5-6-5-5-6, assigning Purva Phalguni (Pooram) to Owl, Vishakha (Visakam) to Crow, and Uttara Ashadha (Uthiradam) to Rooster. Both 1919 and 1930 classical texts (*Pancha Pakshi Rathinam* p. 46) explicitly corroborate 5-6-5-5-6 as the ancient canonical Siddha partition. Which nakshatra boundary is canonical for the app's default birth bird? | 🥇 2025 Workshop Day 1 (`pp-class-01.md` @ 29:00), 🥇 Master's Book (p. 7), 🥈 1930 *Rathinam* (pp. 46–47) vs Shipped `PakshiCalculator._brightVultureNakshatras` | CONFIRMED: canonical partition is **5-6-5-5-6** (Vulture 5, Owl 6, Crow 5, Cock/Rooster 5, Peacock 6) — UNANIMOUS across all 3 lineage sources (workshop @29:00, master book p.7 venba, *Rathinam* pp.46–47 verbatim; internally corroborated by Murugan's Visakam→Crow). Reassign **Pooram/Purva Phalguni → Owl**, **Visakam/Vishakha → Crow**, **Uthiradam/Uttara Ashadha → Rooster**. Shipped 5-5-5-5-7 (Pulippani/vedastro, a modern secondary source) is **overruled**. | ENGINE FIX (Phase 2b): replace `PakshiCalculator` star partition with 5-6-5-5-6. Changes the birth bird for Pooram/Visakam/Uthiradam natives → requires existing-user on-load recalc + regression test (v1.4.1 re-migration lesson). | ✅ Confirmed (owner, 3-source unanimous) |
| CONF-PP-002 | 🔴 T1 | Permanent Natal Bird vs Dark Half Reverse Swap | The shipped engine (`PakshiCalculator.birthBirdFromNakshatraAndPaksha` / Pulippani Table 1 & 2) uses two distinct star tables: Table 1 (Ashwini forward) for Shukla births and Table 2 (Revati backward) for Krishna births. The 2025 workshop teacher explicitly states that for natives with a known birth star (Janma Nakshatra), the Valarpirai table is the permanent lifetime bird ("ஜென்ம பக்ஷி... இந்த ஒரே பறவையை வாழ்நாள் முழுவதும் பயன்படுத்தலாம்... தேய்பிறை பக்ஷி போகவே வேண்டாம்"), and that waxing/waning bird swapping ONLY applies to those deriving their bird from their name initial letter (Nama Pakshi). Both Healer Baskar (`pp-yt-01.md` @ 01:10) and Astrologer S. Gopalakrishnan (`pp-yt-03.md` @ 02:47) corroborate that DOB/birth star yields a single permanent invariant bird, while waning/waxing splitting is exclusively reserved for the name initial method (with folk practitioners often directly mapping calling-name vowels to the 5 elemental birds). Should birth star lookups use a single permanent table for all births, or retain dual Shukla/Krishna tables? | 🥇 2025 Workshop Day 1 (`pp-class-01.md` @ 33:00), 🥇 Master's Book (p. 8), 🥈 1930 *Rathinam* (p. 46, single table), 🥉 YouTube 01 (@ 01:10) & 03 (@ 02:47) vs `calculation-methodology.md` §1 | CONFIRMED: for a KNOWN birth star (Janma Pakshi), use a **SINGLE PERMANENT Valarpirai (bright-half) table — NO Krishna reverse-swap** (workshop @33:00 *"தேய்பிறை பக்ஷி போகவே வேண்டாம்"*; master book p.8; *Rathinam* provides only ONE star table + notes dual tables "fail in practice"; both YouTube masters concur). The waxing/waning swap applies **ONLY to the name-initial (Nama Pakshi) fallback** path (unknown birth star). | ENGINE FIX (Phase 2b): neutralize `birthPaksha` in the star lookup (always single permanent 5-6-5-5-6 table); move the waxing/waning swap to a new `birthBirdFromNameInitialAndPaksha()` fallback. Same existing-user recalc + regression test as CONF-PP-001. | ✅ Confirmed (owner, 4-source) |
| CONF-PP-003 | 🟡 T2 | Natural Friendship & Enmity Matrices | In `pakshi_attributes.dart` (Pulippani), Vulture's friends are Crow & Owl, and enemies are Peacock & Rooster. In the 2025 workshop (Day 3), Vulture's friends in Valarpirai are Peacock & Owl, and enemies are Crow & Rooster; and Vulture ↔ Peacock is declared a permanent invariant ally across both moon phases, while Vulture ↔ Rooster is declared a permanent bitter enemy. Several friend/enemy pairs are inverted between the workshop and Pulippani. Note: 1930 *Pancha Pakshi Rathinam* (p. 46) explicitly comments that dual Shukla/Krishna enmity tables fail in practice and affirms unified natural affinities. Which relational matrix should the app display in compatibility and daily advice? | 🥇 2025 Workshop Day 3 (`pp-class-03.md` @ 02:46), 🥈 1930 *Rathinam* (p. 46) vs `pakshi_attributes.dart` lines 197–252 | CONFIRMED (corpus): use the **unified natural affinity matrix** — Vulture friends = **Peacock & Owl**, enemies = **Crow & Rooster**; **Vulture↔Peacock = permanent ally, Vulture↔Rooster = permanent enemy**. Workshop + *Rathinam* (which states dual Shukla/Krishna enmity tables "fail in practice") overrule Pulippani's inverted pairs. Consistent with CONF-PP-002 (no dual-table). | ENGINE FIX (Phase 2b): correct the friend/enemy tables in `pakshi_attributes.dart`; unify (no phase split). | ✅ Confirmed (owner) |
| CONF-PP-004 | 🟡 T2 | Elemental Ruling Planets | In `pakshi_attributes.dart`, the ruling planets are: Vulture=Saturn, Owl=Mars, Crow=Venus, Rooster=Jupiter, Peacock=Mercury. In the 2025 workshop (Day 4 @ 48:41), the ruling planets are: Vulture=Guru (Jupiter), Owl=Sukran (Venus), Crow=Sevvai (Mars), Rooster=Budhan (Mercury), Peacock=Shani (Saturn), explained by classical Tamil Siddha elemental physics. Which planet mapping governs the bird attributes? | 🥇 2025 Workshop Day 4 (`pp-class-04.md` @ 48:41) vs `pakshi_attributes.dart` lines 91–121 | CONFIRMED (corpus): ruling planets are **Vulture=Guru/Jupiter, Owl=Sukran/Venus, Crow=Sevvai/Mars, Rooster=Budhan/Mercury, Peacock=Shani/Saturn**, derived from the bird→element→planet Siddha logic (not the shipped Pulippani mapping). Overrules `pakshi_attributes.dart`. | ENGINE FIX (Phase 2b): update `PakshiAttributes` planet getters + dependent colour associations. | ✅ Confirmed (owner) |
| CONF-PP-005 | 🟢 T3 | Cardinal Direction Assignments | In `pakshi_attributes.dart`, bird directions are static (Vulture=West, Owl=North, Crow=South, Rooster=East, Peacock=Center). In the 2025 workshop (Day 3 @ 18:21), directions are dynamic by moon phase: Valarpirai (Vulture=East, Owl=South, Crow=West, Rooster=North, Peacock=Sky/Center) and Theipirai (Vulture=East, Owl=North, Crow=South, Rooster=Center, Peacock=West). Should the app support directional tactics with phase-dependent cardinal directions? | 🥇 2025 Workshop Day 3 (`pp-class-03.md` @ 18:21) vs `pakshi_attributes.dart` lines 124–154 | CONFIRMED (corpus): directions are **phase-dependent (dynamic by moon phase)**, not static. Valarpirai: Vulture=East, Owl=South, Crow=West, Rooster=North, Peacock=Sky/Center. Theipirai: Vulture=East, Owl=North, Crow=South, Rooster=Center, Peacock=West. Replace the static assignments. | ENGINE FIX (Phase 2b): make bird direction a function of (bird, paksha) in `pakshi_attributes.dart`; feed the tactical/compass feature. | ✅ Confirmed (owner) |
| CONF-PP-006 | 🟡 T2 | Sub-Yama (அந்தர புத்தி) Duration: Uniform 28.8 min vs Classical Unequal Weighted Slices | The shipped engine and modern workshops divide each 144-minute Yama into 5 equal 28.8-minute sub-yamas (1.2 நாழிகை each). The 1919 and 1930 classical texts (*Agathiyar Panja Pakshi Sasthiram* pp. 5–6 & *Pancha Pakshi Rathinam* p. 12, 40) preserve an unequal weighted distribution based on intrinsic activity potency: Arasu=2.0 நாழிகை (48m), Nadai=1.5 நாழிகை (36m), Oon=1.25 நாழிகை (30m), Thuyil=0.75 நாழிகை (18m), Saavu=0.50 நாழிகை (12m), summing to 6.0 நாழிகை (144m). Should the engine support an optional or default classical weighted sub-yama calculation? | 🥈 1919 Classic (pp. 5–6), 🥈 1930 *Rathinam* (p. 12, 40) vs Shipped `PakshiCalculator._subYamaDuration` | CONFIRMED (owner — SUPPORT BOTH): this is a genuine modern-vs-classical divergence (the 2025 workshop + shipped app use **equal 28.8-min** sub-yamas; the 1919/1930 classics use **potency-weighted** Arasu 48 / Nadai 36 / Oon 30 / Thuyil 18 / Saavu 12 min). Owner decision: **DEFAULT to equal 28.8-min (the workshop/lineage the app follows), and offer the classical weighted model as a user-selectable OPTION** (labelled "Classical / Agathiyar-Rathinam weighting"). Do not silently switch; let the user choose. | ENGINE FIX (Phase 2b): make `_subYamaDuration` support two modes (equal default + classical weighted); add a settings toggle. | ✅ Confirmed (owner — support both, equal default) |
