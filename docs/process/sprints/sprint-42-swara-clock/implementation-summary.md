...
| 42.2 Weekday Udhaya dawn seed (CONF-013/001) | `lib/features/astro_engine/domain/swara_clock.dart` | ✅ | `SwaraClock.seedFor(weekday, paksha)` implementing CONF-013 / CONF-001 with Thursday paksha split: Shukla (Waxing) = Ida / Left (1h), Krishna (Waning) = Pingala / Right (2h). |
...
| 42.7 Bilingual EN/TA | `app_en.arb`, `app_ta.arb` | ✅ | Added `nightSwaraRest` in both EN & TA. Pruned orphaned `nightNoNostrilPattern`. 100% key parity maintained. |
...
| 5 | Thursday | **Shukla → Left (Lunar) 1h · Krishna → Right (Solar) 2h** | — |
...
- `grep -rn "expectedFlowForYama\|NostrilPattern" lib` → **0 live call sites** in application logic.
  - Only `lib/features/astro_engine/domain/nostril_pattern.dart` (the deprecated shim declaration) appears in the search; orphaned ARB key `nightNoNostrilPattern` pruned.
