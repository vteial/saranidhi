...
| 🟢 | 📋 → S43 | **BUG-v1.11.1-02 — Best Times card yama prefix not localized.** The Home "Best Times This Week" card hardcoded `'Y${entry.yamaNumber}'` (`best_times_card.dart:183`), so Tamil mode showed `Y1` while the Analytics screen already used the localized `yamaShortPrefix` (`யா`). Same class as BUG-v1.11.1-01 / BUG-v1.10.1-01 (value not localized). **Fix:** `'${l10n.yamaShortPrefix}${entry.yamaNumber}'` (`l10n` already in scope). Owner-found (2026-09-14). **Folded into Sprint 43 (v1.11.1) Task 43.5** mid-flight. |
